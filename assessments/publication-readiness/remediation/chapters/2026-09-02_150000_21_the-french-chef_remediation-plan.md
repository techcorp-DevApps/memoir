# Chapter Remediation Plan — 21 THE FRENCH CHEF IN AN ITALIAN KITCHEN

- **Source file**: `chapters/21_TheFrenchChef.md`
- **Current readiness**: PUBLICATION READY — MINOR EDIT
- **Target readiness**: PUBLICATION READY
- **REM items**: REM-015, REM-016; plus governing-table items sourced from this chapter (REM-032, REM-033, executed in `CLAUDE.md`, not here)
...
- **Entry bridge (from 20)**: adequate — restates, in fuller detail, the career move ch.20 already summarised. This is the same recap-redundancy pattern as REM-012 (awareness-only, no mandatory fix) — do not trim without Bosco's explicit request.
- **Exiting bridge (to 22)**: ch.22 (`Dave`) is currently an unwritten stub — this chapter cannot be faulted for the absence of a textual handoff to content that doesn't exist yet. The closing line above already leaves the right kind of open social terrain for a friendship arc to land in; no action needed here regardless of when ch.22 is eventually drafted.
- **Coordinated change needed elsewhere**: `CLAUDE.md` §5 for REM-032 (Carly/Rachael) and REM-033 (Scotty/Raj rows) — both sourced from this chapter but executed only in the governing file.

## Governing dependencies

- REM-016: `CLAUDE.md` §5's Dylan row is held pending this chapter's clarification (do not add the row first).
- REM-032: `CLAUDE.md:202` (Carly/Rachael), pending Bosco's sign-off — no chapter edit under any resolution direction.
- REM-033: `CLAUDE.md` §5 (Scotty/Raj rows) — pure addition, no chapter edit needed.
